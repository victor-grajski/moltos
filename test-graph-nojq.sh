...
curl -s -X POST "$BASE_URL/connections" \
  ...
  -d '{"agentId":"Alice","targetId":"Bob","type":"follow","howMet":"Met at conference"}'
echo ""

echo "  - Bob and Charlie are mutual connections..."
curl -s -X POST "$BASE_URL/connections" \
  -H "Content-Type: application/json" \
  -d '{"agentId":"Bob","targetId":"Charlie","type":"connection","notes":"Close friends"}'
echo ""

echo "  - Charlie and Diana are collaborators..."
curl -s -X POST "$BASE_URL/connections" \
  -H "Content-Type: application/json" \
  -d '{"agentId":"Charlie","targetId":"Diana","type":"collaborator","howMet":"Worked on project together","sharedProjects":["MoltOS"]}'
echo ""

echo "  - Diana and Eve are connected..."
curl -s -X POST "$BASE_URL/connections" \
  -H "Content-Type: application/json" \
  -d '{"agentId":"Diana","targetId":"Eve","type":"connection"}'
echo ""

echo "  - Alice and Frank are collaborators..."
curl -s -X POST "$BASE_URL/connections" \
  -H "Content-Type: application/json" \
  -d '{"agentId":"Alice","targetId":"Frank","type":"collaborator"}'
echo ""

# Test 4: Get connections for an agent
echo "4. Getting connections for Bob..."
curl -s "$BASE_URL/connections/Bob"
echo ""
